...
/**
 * Mojo to create ZIP packages of the project build artifacts.
 */
@Mojo( name = MSBuildProjectPackageMojo.MOJO_NAME,
        defaultPhase = LifecyclePhase.PACKAGE )
@Execute( phase = LifecyclePhase.PACKAGE )
public class MSBuildProjectPackageMojo extends AbstractMSBuildPluginMojo
{
    /**
     * The name this Mojo declares, also represents the goal.
     */
    public static final String MOJO_NAME = "project-package";
    ...
    public final void doExecute() throws MojoExecutionException, MojoFailureException 
    {
        ...
        if ( MSBuildPackaging.isProject( mavenProject.getPackaging() ) )
        {
            buildAndattachProjectArtifacts();
        }
        else
        {
            getLog().warn( "Packaging is not " + MSBuildPackaging.MSBUILD_PROJECT + ", no packaging to do." );
        }
        
    }

    private void buildAndattachProjectArtifacts() throws MojoExecutionException
    {
